...
test('latency storage is service-only, fenced to a running lease, and holds no message content', async () => {
  const db = new PGlite();
  try {
    ...
    assert.equal((await db.query<{ ok: boolean }>('select public.record_chat_latency_measurement($1,$2,0,4,7,8,null,null,null,null,42) as ok', [run.id, claim.value.version])).rows[0].ok, true);
    const { rows: [measurement] } = await db.query<{ worker_claimed_ms: number; executor_finished_ms: number }>('select worker_claimed_ms,executor_finished_ms from public.chat_latency_measurements where run_id=$1', [run.id]);
    assert.deepEqual(measurement, { worker_claimed_ms: 0, executor_finished_ms: 42 });
