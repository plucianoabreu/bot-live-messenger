...
async function setup() {
  await psql([], `drop schema public cascade; create schema public; create schema auth;
...
try {
  ...
  const run = (await psql(['--tuples-only', '--no-align'], `select set_config('request.jwt.claim.sub','${owner}',false);
    select public.enqueue_message((select id from public.bots where user_id='${owner}' limit 1),'lock test','${randomUUID()}');`)).trim().split('\n').at(-1);
  if (!run) throw new Error('PREWARM_TEST_RUN_MISSING');
  await psql([], `select public.claim_chat('${run}');`);
  const race = await concurrent({
    left: `begin; select public.claim_prewarm_cleanup('${intent}'); select pg_sleep(2); commit;`,
    right: `select public.claim_hermes_workspace('${run}',1,'${'a'.repeat(64)}');`,
  });
  if (race.elapsed < 1_800 || !/PREWARM_PENDING|PREWARM/.test(race.b)) throw new Error('PREWARM_HANDOFF_CLEANUP_LOCK_NOT_PROVEN');
