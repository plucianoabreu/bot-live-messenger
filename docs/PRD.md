...
- USD 50 pilot override: five chat messages and one computer task per account, shared across bots, without automatic renewal. Reserve USD 0.02 per chat run and USD 0.25 per computer run, including all delegated work. Global allocation pools are USD 10 for chat and USD 25 for computer tasks; USD 15 remains outside execution for infrastructure, tests and discrepancies. All paid tests must count against these pools or the reserve.
